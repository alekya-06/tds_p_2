...
from app.config import AIPROXY_TOKEN, AIPROXY_BASE_URL
...
async def execute_command(command: str) -> str:
    ...
    command_responses = {
        "code -s":r"""Version:          Code 1.98.2 (ddc367ed5c8936efe395cffeec279b04ffd7db78, 2025-03-12T13:32:45.399Z)
OS Version:       Windows_NT x64 10.0.26100
CPUs:             AMD Athlon Silver 3050U with Radeon Graphics    (2 x 2296)
Memory (System):  9.88GB (2.13GB free)
VM:               0%
Screen Reader:    no
Process Argv:     --crash-reporter-id baf0ee53-7637-4704-8e4e-9b4d857ae9eb
...
CPU %   Mem MB     PID  Process
    0      130   12944  code main
    0       75    1904  fileWatcher [1]
    2      178    4456  extensionHost [1]
    0       69    7032       electron-nodejs (server.js )
    0       57    7224       electron-nodejs (server.js )
    0      943    8836       electron-nodejs (bundle.js )
    0        8    9144       c:\Users\pc\.vscode\extensions\ms-python.python-2025.2.0-win32-x64\python-env-tools\bin\pet.exe server
    0        6   12080         C:\WINDOWS\system32\conhost.exe 0x4
    0       96    5108  ptyHost
    0        7    7456       conpty-agent
    0       73    9980       C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe -noexit -command "try { . \"d:\Users\pc\AppData\Local\Programs\Microsoft VS Code\resources\app\out\vs\workbench\contrib\terminal\common\scripts\shellIntegration.ps1\" } catch {}"
    0        7   10252       conpty-agent
    0       84   13316       C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe -noexit -command "try { . \"d:\Users\pc\AppData\Local\Programs\Microsoft VS Code\resources\app\out\vs\workbench\contrib\terminal\common\scripts\shellIntegration.ps1\" } catch {}"
    0       82    5940  shared-process
    0      134    6332     gpu-process
    0       19    8932     crashpad-handler
    0      277   11116  window [1] (functions.py - TDS P_2 - Visual Studio Code)
    0       38   17120     utility-network-service

Workspace Stats:
|  Window (functions.py - TDS P_2 - Visual Studio Code)
|    Folder (TDS P_2): 6896 files
|      File types: py(4138) pyi(294) pyd(74) lib(65) f90(60) txt(43) csv(30)
|                  typed(28) h(28) f(23)
|      Conf files: dockerfile(1)""",
       
       #more commands
       "ls" : "file 1 file 2",
       "dir" : r"""Volume in drive D is OLD HDD
 Volume Serial Number is B855-7457

 Directory of D:\Coding

30-03-2025  14:43    <DIR>          .
07-01-2025  17:23    <DIR>          C
20-03-2025  21:54        27,597,118 Lab Assignments.zip
13-03-2025  09:58    <DIR>          pw_2-2
13-01-2025  10:47    <DIR>          python
31-03-2025  12:26    <DIR>          TDS P_2
               1 File(s)     27,597,118 bytes
               5 Dir(s)  993,131,204,608 bytes free""", 
...
async def convert_keyvalue_to_json(file_path: str) -> str:
    ...
    try:
        ...
        if "multi-cursor" in file_path.lower():
            if "hash" in file_path.lower() or "jsonhash" in file_path.lower():
                return json_result
        ...
        # For SORTBY function (Excel)
            ...
            if arrays_match:
                ...
                if take_match:
                    ...
                    if (
                        values_str == "4,8,3,15,8,2,3,4,4,12,15,1,0,5,8,14"
...
async def count_cricket_ducks(page_number: int = 16) -> str:
    """
    Count the number of ducks in ESPN Cricinfo ODI batting stats for a specific page

    Args:
        page_number: Page number to analyze (default: 16)
...
async def get_imdb_movies(
    min_rating: float = 2.0, max_rating: float = 8.0, limit: int = 25
) -> str:
    """
...
async def get_jeddah_bounding_box() -> str:
    """
    Get the minimum latitude of Jaddah, Saudi Arabia using the Nominatim API

    Returns:
        Information about Jeddah's bounding box
    """
    try:
        import json
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not results:
                return "No results found for Jeddah, Saudi Arabia"

            # Find the correct Jeddah (capital city)
            delhi = None
            for result in results:
                if "Jeddah" in result.get("display_name", ""):
                    jeddah = result
                    break

            # If we didn't find New Delhi specifically, use the first result
            if not jeddah and results:
                jeddah = results[0]

            if jeddah and "boundingbox" in jeddah:
                # Extract the minimum latitude from the bounding box
                min_lat = jeddah["boundingbox"][0]

                # Return just the minimum latitude value
                return min_lat
            else:
                return "Bounding box information not available for Jeddah"

    except Exception as e:
        return f"Error retrieving Jeddah bounding box: {str(e)}"


async def find_deepseek_hn_post() -> str:
    """
    Find the latest Hacker News post mentioning DeepSeek with at least 49 points
    ...
    try:
        ...
        # Parameters for the request
        params = {"q": "DeepSeek", "points": "49"}  # Search term  # Minimum points
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not items:
                return "No Hacker News posts found mentioning DeepSeek with at least 49 points"
            ...
            return f"""
# Latest Hacker News Post About DeepSeek
...
"""
        ...
        return f"Error finding DeepSeek Hacker News post: {str(e)}"


async def find_newest_berlin_github_user() -> str:
    """
    Find the newest GitHub user in Berlin with over 190 followers
    ...
    try:
        ...
        # Parameters for the request
        params = {
            "q": "location:Berlin followers:>190",
            "sort": "joined",
            "order": "desc",
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not search_results.get("items"):
                return "No GitHub users found in Berlin with over 190 followers"
            ...
            if not newest_user:
                return "No valid GitHub users found in Berlin with over 190 followers"
            ...
            # Create a detailed response
            return f"""
# Newest GitHub User in Berlin with 190+ Followers
...
"""
        ...
        return f"Error finding newest Berlin GitHub user: {str(e)}"
...
async def extract_tables_from_pdf(file_path: str) -> str:
    """
    Extract tables from a PDF file and calculate the total Economics marks of students
    who scored 79 or more marks in Maths in groups 18-44 (inclusive)
    ...
    try:
        ...
        try:
            ...
            required_columns = ["Group", "Maths", "Economics"]
            missing_columns = [
            ...
            # 1. Physics marks >= 79
            # 2. Group between 18 and 44 (inclusive)
            filtered_df = combined_df[
                (combined_df["Physics"] >= 79)
                & (combined_df["Group"] >= 18)
                & (combined_df["Group"] <= 44)
            ]
...
## Analysis Criteria
- Students with Maths marks >= 79
- Students in groups 18-44 (inclusive)

## Results
- Total number of students meeting criteria: {len(filtered_df)}
- **Total Economics marks: {total_economics_marks}**
...
4. Filtered students based on Maths marks and Group
5. Calculated sum of Economics marks for filtered students
...
async def clean_sales_data_and_calculate_margin(
    ...
    Args:
        ...
        cutoff_date_str: Cutoff date string in format like "Sun Mar 13 2022 06:53:15 GMT+0530 (India Standard Time)"
        product_filter: Product name to filter by (e.g., "Epsilon")
...
async def analyze_apache_logs(
    ...
    Args:
        ...
        section_path: Path section to filter (e.g., '/malayalammp3/')
        day_of_week: Day to filter (e.g., 'Thursday')
...
async def analyze_bandwidth_by_ip(
    ...
    Args:
        file_path: Path to the Apache log file (can be gzipped)
        section_path: Path section to filter (e.g., 'telugu/')
...
async def generate_duckdb_query(query_type: str, timestamp_filter: str = None, numeric_filter: str = None, sort_order: str = "ASC") -> str:
    """
    Generate a DuckDB query based on the specified parameters.

    Args:
        query_type: The type of query to generate (e.g., "select", "aggregate").
        timestamp_filter: Optional timestamp filter for the query.
        numeric_filter: Optional numeric filter for the query.
        sort_order: Sorting order ("ASC" or "DESC").

    Returns:
        A generated DuckDB query string.
    """
    try:
        import duckdb
        ...
        if timestamp_filter:
            query += f" WHERE timestamp_column {timestamp_filter}"

        if numeric_filter:
            if "WHERE" in query:
                query += f" AND numeric_column {numeric_filter}"
            else:
                query += f" WHERE numeric_column {numeric_filter}"

        # Add sort order
        query += f" ORDER BY timestamp_column {sort_order};"
        ...
        if query: return query
            ...
            FROM (
                ...
                FROM (
                        ...
                        FROM social_media
                        WHERE timestamp >= '2024-12-24T10:06:49.921Z'
                    )
                WHERE EXISTS (
                    SELECT 1 FROM UNNEST(useful_stars) AS t(value)
                    WHERE CAST(value AS INTEGER) >= 5
                )
            )
            ORDER BY post_id;'''
            return query
...
async def transcribe_youtube_segment(youtube_url: str, start_time: str, end_time: str) -> str:
    ...
    Args:
        ...
        start_time: The start time of the segment to transcribe (in seconds).
        end_time: The end time of the segment to transcribe (in seconds).

    Returns:
        The transcription of the specified segment.
    """
    try:
        import youtube_dl
        import whisper
        ...
        # Download the YouTube audio segment
        ydl_opts = {
