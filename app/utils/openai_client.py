...
async def send_openai_request(question: str) -> str:
    try:
        AIPROXY_TOKEN = os.getenv("AIPROXY_TOKEN")
        AIPROXY_BASE_URL = os.getenv("AIPROXY_BASE_URL")
        ...
        response_content = await send_openai_request(question)
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "query_type": {
                            "type": "string",
                            "description": "Type of query to generate (e.g., 'post_id', 'post_comment')",
                        },
                        "timestamp_filter": {
                            "type": "string",
                            "description": "ISO timestamp for filtering data (e.g., '2025-01-30T22:48:37.211Z')",
                        },
                        "numeric_filter": {
                            "type": "integer",
        ...
        if "tool_calls" in message:
            for tool_call in message["tool_calls"]:
                ...
                if function_name == "execute_command":
                    ...
                    answer = await get_jeddah_bounding_box()

                elif function_name == "find_duckdb_hn_post":
                    answer = await find_deepseek_hn_post()

                elif function_name == "find_newest_seattle_github_user":
                    answer = await find_newest_berlin_github_user()
